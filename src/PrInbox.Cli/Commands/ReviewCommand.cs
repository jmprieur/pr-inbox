...
{
    [CommandArgument(0, "<PR_ID>")]
    [Description("PR display id, e.g. gh.com:owner/repo#42")]
...
internal sealed class ReviewCommand : AsyncCommand<ReviewSettings>
{
    public override async Task<int> ExecuteAsync(CommandContext context, ReviewSettings settings)
    {
        ...
        var row = await prRepo.GetAsync(settings.PrId, CancellationToken.None);
        if (row is null)
        {
            AnsiConsole.MarkupLine($"[red]PR '{Markup.Escape(settings.PrId)}' not found in inbox.[/] Run [bold]pr-inbox sync[/] first.");
            return 1;
        }
        ...
        var safePr = MakeSafePathSegment(row.Identity.Display);
        ...
        var metadata = new
        {
            pr_identity = row.Identity.Display,
        ...
        await prRepo.UpdateLastBriefedAsync(row.Identity.Display, snapshot.HeadSha, CancellationToken.None);
        ...
        IReadOnlyList<ReviewRunRow> priorRuns)
    {
        ...
        sb.AppendLine($"# Review brief — {pr.Identity.Display}");
        sb.AppendLine();
        ...
        sb.AppendLine($"- **Display id:** `{pr.Identity.Display}`");
        ...
        sb.AppendLine("- **DO NOT post** anything to GitHub directly. Curate findings and route them through me.");
        sb.AppendLine("- For each finding, declare `diff_anchorable: true|false` so I can route inline vs body.");
        sb.AppendLine("- Apply the **95%+ inline filter**: only post as inline what is high-confidence on the right line of the right file. Architectural / judgment-call findings go in the body.");
        sb.AppendLine();
        sb.AppendLine("## Staleness clause");
        sb.AppendLine();
        sb.AppendLine($"Before posting, verify PR HEAD is still `{snapshot.HeadSha}`. If not, re-run `pr-inbox review {pr.Identity.Display}` to get a fresh brief.");
        sb.AppendLine();

        return sb.ToString();
    }

    private static string MakeSafePathSegment(string display)
    {
