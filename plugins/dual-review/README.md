...
| Kind | Name | Purpose |
|---|---|---|
| Agent | [`dual-model-review`](agents/dual-model-review.md) | Orchestrates one round of review by two independent primary reviewers (required: one Opus-family + one GPT-family model) over the same change set, then cross-references their findings into a single verdict + de-duplicated finding list. Local reviewers are shadow-only and never satisfy primary quorum. One round per call; the caller iterates. |
